...
def show_range_image(frame, lidar_name):
    ...
    ####### ID_S1_EX1 START #######     
    #######
    print("student task ID_S1_EX1: lidar: ", lidar_name)
    ...
    ri_intensity = ri[:,:,1]
    percent1 = np.percentile(ri_intensity, 1)
    percent99 = np.percentile(ri_intensity, 99)
    ri_intensity = ri_intensity * 255 / (percent99-percent1)  
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_cpy[lidar_pcl_cpy[:,3] > 1.0] = 1.0
    ...
    _, idx_intensity_unique, count = np.unique(lidar_pcl_top[:, 0:2], axis=0, return_index=True, return_counts=True)
